...
export default {
  ...
  methods: {
    ...
    processPayment(token) {
      ...
      axios.post(
          'https://southerntiersummit.com/api/test.php?stripeToken=' + token + '&stripeAmt=4900&description=Registration for the Industry Summit hosted by Southern Tier 8.&email=' + this.$store.state.temp_user.email + '&first_name=' + this.$store.state.temp_user.first_name + '&last_name= '+ this.$store.state.temp_user.last_name + '&zip' + this.$store.state.temp_user.zip + ' &company=' + this.$store.state.temp_user.position + '&customerID=' + this.$store.state.customerID)
        .then(response => {
